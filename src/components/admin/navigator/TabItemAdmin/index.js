...
export default function TabItemAdmin({ title, active, onPress, onLongPress }) {
  const Icon = () => {
    if (title === 'Beranda') {
      ...
      return active ? <Ionicons name="add-circle" color={colors.green1} size={25} /> : <Ionicons name="add-circle-outline" color={colors.grey} size={25} />;
    } if (title === 'Profil') {
      return active ? <Ionicons name="person" color={colors.green1} size={25} /> : <Ionicons name="person-outline" color={colors.grey} size={25} />;
    } if (title === 'Pengaturan') {
